...
게임잡·원티드·사람인·잡코리아에서 **기획 / 클라이언트 / 서버 / 아트** 공고를 모아
...
직군마다 채널을 따로 두면 `#클라이언트` 만 알림 켜기, 직군별로 과거 공고 찾아보기가 됩니다.

1. 디스코드에서 채널 4개를 만듭니다. 이름에 직군이 들어가기만 하면 됩니다 —
   `기획` `클라이언트` `서버` `아트`, 또는 `공고-기획` `💼기획` 같은 것도 잡힙니다.
2. 봇이 그 채널들에 글을 쓸 수 있는지 확인합니다.
3. 채널 ID를 찾아 연결합니다. 손으로 복사할 필요 없이 자동으로 찾아줍니다.

```bash
python setup_channels.py
```

`config.yaml` 의 `channels` 에 ID가 채워집니다. GitHub Actions 로 돌린다면 커밋해서 올리세요.

```bash
git add config.yaml
```
```bash
git commit -m "직군별 채널 연결"
```
```bash
git push
```

채널을 지정하지 않은 직군은 `.env` 의 `DISCORD_CHANNEL_ID` 로 갑니다.
아무것도 지정하지 않으면 예전처럼 한 채널에 전부 옵니다.
...
예를 들어 아트에 원화(5)·이펙트(8)를 더 넣고 싶으면
`"아트": ["6", "7", "5", "8"]` 로 고치면 됩니다.
